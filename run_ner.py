...
def evaluate(args, model, tokenizer, labels, pad_token_label_id, mode, prefix=""):
    ...
    sent_accuracy = np.mean([out_label_list[x] == preds_list[x] for x in range(len(preds_list))])
    results = {
        "loss": eval_loss,
...
def load_and_cache_examples(args, tokenizer, labels, pad_token_label_id, mode):
    ...
    if os.path.exists(cached_features_file) and not args.overwrite_cache:
        ...
        examples = read_examples_from_file(args.data_dir, mode, file_name)
...
def main():
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        output_eval_file = os.path.join(args.output_dir, args.dev_file.split('.')[0] + "_eval_results.txt")
    ...
    if args.do_predict and args.local_rank in [-1, 0]:
        ...
        # Save results
        output_test_results_file = os.path.join(args.output_dir, args.test_file.split('.')[0] + "_test_results.txt")
        with open(output_test_results_file, "w") as writer:
            for key in sorted(result.keys()):
                writer.write("{} = {}\n".format(key, str(result[key])))
        # Save predictions
        output_test_predictions_file = os.path.join(args.output_dir, args.test_file.split('.')[0] +
